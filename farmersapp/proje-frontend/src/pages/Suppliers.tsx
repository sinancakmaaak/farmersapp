...
const Suppliers: React.FC = () => {
  ...
  const loadSuppliers = async () => {
    ...
    try {
      ...
      setError('Tedarikçi bilgileri yüklenirken bir hata oluştu.');
      ...
      <div className="sm:flex sm:items-center mb-6">
        <div className="sm:flex-auto">
          ...
          <p className="mt-2 text-sm text-gray-700">
            Tüm tedarikçilerin listesi ve yönetimi
          </p>
        </div>
            ...
            <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
              <table className="min-w-full divide-y divide-gray-300">
                <thead className="bg-gray-50">
                  <tr>
                    <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Firma Adı</th>
                    <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">İletişim Kişisi</th>
                    ...
                    <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Adres</th>
                ...
                <tbody className="divide-y divide-gray-200 bg-white">
                  {suppliers.map((supplier) => (
                    <tr key={supplier.id}>
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-900">{supplier.companyName}</td>
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-900">{supplier.contactPerson}</td>
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-900">{supplier.phoneNumber}</td>
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-900">{supplier.email}</td>
                      <td className="px-3 py-4 text-sm text-gray-900">{supplier.address}</td>
          ...
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mt-5 sm:mt-6 sm:grid sm:grid-cols-2 sm:gap-3">
                ...
                >
                  {isLoading ? 'Kaydediliyor...' : selectedSupplier ? 'Güncelle' : 'Ekle'}
                </button>
                <button
                  type="button"
